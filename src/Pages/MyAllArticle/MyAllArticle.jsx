...
const MyAllArticle = ({ myArticle, refetch, axiosScure }) => {
  ...
  const handleApprove = (articleId) => {
    axiosScure.patch(`/articles/approve/${articleId}`).then((res) => {
  ...
  const handleDecline = () => {
    ...
    axiosScure.patch(`/articles/decline/${modalArticle._id}`, { reason: declineReason })
      .then((res) => {
        if (res.data.modifiedCount > 0) {
  ...
  const handleMakePremium = (articleId) => {
    axiosScure.patch(`/articles/make-premium/${articleId}`).then((res) => {
      if (res.data.modifiedCount > 0) {
        refetch();
        Swal.fire({
          position: "top-end",
  ...
  const handleDelete = (articleId) => {
    axiosScure.delete(`/articles/${articleId}`).then((res) => {
